...
const loginApiUrl = testsHelper.loginApiUrl;
const usersApiURl = testsHelper.usersApiURl;
const blogsApiUrl = testsHelper.blogsApiUrl; //in oder not to the repeat the url too much
...
describe(`TESTS FOR blogs onAPI: `, () => {
    ...
    before(async () => {
        ...
        const { username, password } = testsHelper.usersToAddBeforeTests[0];
        await Promise.all(testsHelper.usersToAddBeforeTests.map(user => testApi.post(usersApiURl).send(user)))
        const loginRequestResponse = await testApi.post(loginApiUrl).send({username, password})
        //console.log(loginRequestResponse.body)
        loginToken = loginRequestResponse.body.loginToken;
    })
    ...
    beforeEach(async () => { //initialize the database
        ...
        //responses is an arrary of responses, not one single response
        const responses = await Promise.all(testsHelper.dummyBlogs.map(blog => testApi.post(blogsApiUrl)
            .send({
                title: blog.title,
                author: blog.author,
                likes: blog.likes
    ...
    test(`Delete by ID should return status 204, and confirm length`, async () => {
        ...
        const deleteResponse = await testApi
            .delete(`${blogsApiUrl}/${blogToDelete._id}`)
            .set({ Authorization: `Bearer ${loginToken}` })
            .expect(204)//btw expect throws an error and stops further execution if its not fulfilled
        const response = await testApi.get(blogsApiUrl);
    ...
    test(`adding a blog will give it an ID for the user that created it`, async () => {
        const response = await testApi.post(blogsApiUrl).send(blogCreationTemplate).set({ Authorization: `Bearer ${loginToken}` });
